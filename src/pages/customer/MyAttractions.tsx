...
const MyAttractions = () => {
  const [purchases, setPurchases] = useState<AttractionPurchase[]>([]);
  const [selectedPurchase, setSelectedPurchase] = useState<AttractionPurchase | null>(null);
  ...
  const fetchPurchases = async () => {
    if (!customerData?.email) {
      setError('Please log in to view your attraction tickets');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const res = await attractionPurchaseService.getCustomerPurchasesByEmail(customerData.email, filters);
      ...
      setError('Failed to load your attraction tickets');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-blue-200/70 text-xs font-semibold uppercase tracking-widest">Attraction Tickets</span>
                </div>
                <h1 className="text-xl font-bold" style={{ color: 'white' }}>My Attractions</h1>
                <p className="text-blue-200/60 text-sm mt-0.5">View and manage your attraction tickets</p>
              </div>
              {totalItems > 0 && (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-5 slide-up">
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              <div className="flex-1 relative max-w-md w-full">
                ...
                <input
                  type="text"
                  placeholder="Search by attraction name, location..."
                  value={searchTerm}
            ...
            <div className="space-y-3">
              {purchases.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">No attraction tickets found</h3>
                  <p className="text-sm text-gray-500 mb-5">
                    {statusFilter !== 'all' ? `No ${statusFilter} tickets to show` : 'Purchase tickets to an attraction'}
                  </p>
                  {statusFilter !== 'all' ? (
                    <button
                  ...
                  >
                    <div className="p-4 sm:p-5">
                      <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                        {/* Thumbnail */}
                        {purchase.attraction?.image && (
                          <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                            <img
                              src={getImageUrl(purchase.attraction.image)}
                              alt={purchase.attraction.name}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2.5 mb-2">
                            <h3 className="text-base font-semibold text-gray-900 truncate">
                              {purchase.attraction?.name || 'Attraction'}
                            </h3>
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-gray-500">
                            ...
                            {purchase.scheduled_time && (
                              <span className="inline-flex items-center gap-1.5">
                                ...
                                {formatTime(purchase.scheduled_time)}
                              </span>
                            )}
                            <span className="inline-flex items-center gap-1.5">
                          ...
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-2.5 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-gray-800">{purchase.attraction?.name || '-'}</span>
                              </div>
                              {purchase.attraction?.pricing_type && (
                                <div className="flex justify-between">
                                  <span className="text-gray-500">Pricing</span>
                                  <span className="text-gray-700 capitalize">{purchase.attraction.pricing_type.replace('_', ' ')}</span>
                                </div>
                              )}
                              <div className="flex justify-between">
                              ...
                              {purchase.scheduled_date && (
                                <div className="flex justify-between">
                                  ...
                                  <span className="text-gray-700">{formatDate(purchase.scheduled_date)}</span>
                                </div>
                              )}
                              {purchase.scheduled_time && (
                                <div className="flex justify-between">
                                  <span className="text-gray-500">Scheduled Time</span>
                                  <span className="text-gray-700">{formatTime(purchase.scheduled_time)}</span>
                                </div>
                              )}
                              {purchase.attraction?.duration && (
                                <div className="flex justify-between">
                                  <span className="text-gray-500">Duration</span>
                                  <span className="text-gray-700">{purchase.attraction.duration} {purchase.attraction.duration_unit || 'minutes'}</span>
                                </div>
                              )}
                              <div className="flex justify-between">
                              ...
                              {purchase.checked_in_at && (
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className="text-blue-700 text-xs font-medium">{formatDate(purchase.checked_in_at)}</span>
                                </div>
                              )}
                              {purchase.notes && (
                          ...
                          {purchase.add_ons && purchase.add_ons.length > 0 && (
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                              ...
                              <div className="space-y-2">
                                {purchase.add_ons.map((addon: { id: number; name: string; price?: number; pivot?: { quantity?: number; price_at_purchase?: number } }) => (
                                  <div key={addon.id} className="flex justify-between text-sm">
                                    ...
                                    <span className="text-gray-700 font-medium">
                                      ${addon.pivot?.price_at_purchase
                                        ? (addon.pivot.price_at_purchase * (addon.pivot.quantity || 1)).toFixed(2)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 fade-in" onClick={() => setShowDetailsModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
              <h2 className="text-lg font-bold text-gray-900">Attraction Ticket Details</h2>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <div className="flex items-start gap-4">
                  {selectedPurchase.attraction?.image && (
                    <div className="w-20 h-20 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                      <img
                        src={getImageUrl(selectedPurchase.attraction.image)}
                        alt={selectedPurchase.attraction.name}
                        className="w-full h-full object-cover"
                      />
                    </div>
                  )}
                  <div>
                    <h3 className="font-semibold text-gray-900 text-lg">{selectedPurchase.attraction?.name || 'Attraction'}</h3>
                    {selectedPurchase.attraction?.location && (
                      <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                        <MapPin size={13} /> {selectedPurchase.attraction.location.name}
                      </p>
                    )}
                    <div className="flex items-center gap-2 mt-2">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-medium rounded-full ${getStatusColor(selectedPurchase.status)}`}>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-3 text-sm">
                ...
                {selectedPurchase.scheduled_date && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Scheduled Date</span>
                    <span className="text-gray-800">{formatDate(selectedPurchase.scheduled_date)}</span>
                  </div>
                )}
                {selectedPurchase.scheduled_time && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Scheduled Time</span>
                    <span className="text-gray-800">{formatTime(selectedPurchase.scheduled_time)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                {selectedPurchase.attraction?.duration && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Duration</span>
                    <span className="text-gray-800">{selectedPurchase.attraction.duration} {selectedPurchase.attraction.duration_unit || 'minutes'}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                {selectedPurchase.checked_in_at && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Checked In</span>
                    <span className="text-blue-700 font-medium">{formatDate(selectedPurchase.checked_in_at)}</span>
                  </div>
                )}
                <div className="border-t border-gray-200 pt-3 flex justify-between">
              ...
              {selectedPurchase.add_ons && selectedPurchase.add_ons.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedPurchase.add_ons.map((addon: { id: number; name: string; price?: number; pivot?: { quantity?: number; price_at_purchase?: number } }) => (
