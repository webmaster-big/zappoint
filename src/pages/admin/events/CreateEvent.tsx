      ...
      <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
          ...
          {!isCompanyAdmin && (
            <div className={`flex items-start gap-2 rounded-lg border px-3 py-2 ${currentUser?.location_id ? `border-${themeColor}-200 bg-${themeColor}-50` : 'border-amber-200 bg-amber-50'}`}>
              ...
              <p className="text-sm text-neutral-700">
                Creating at <span className="font-semibold text-neutral-900">{currentUser?.location_name || 'no location assigned'}</span>
              </p>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Time slot length (min) *</label>
              <input
                type="number"
                min={5}
                ...
                required
              />
              <p className="mt-1 text-xs text-gray-500">Each session is this long, and the next one starts as soon as it ends.</p>
            </div>
          </div>
          )}
        </div>
